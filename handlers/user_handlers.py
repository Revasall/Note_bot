...
async def process_complete_creating(callback: CallbackQuery, state: FSMContext):
    note = await state.get_data()
    users_db[callback.from_user.id]['notes'][note['title_note']] = note
    print(users_db)
    await state.clear()
...
@router.message(Command(commands='chose_note'), StateFilter(default_state))
async def process_chose_note_command(message: Message, state: FSMContext):
    if users_db[message.from_user.id]['notes']:
        await message.answer(
            text=LEXICON['/chose_note'],
            reply_markup=list_of_note_keyboard(*users_db[message.from_user.id]['notes'].keys())
            )
        print(*users_db[message.from_user.id]['notes'].keys())
...
@router.callback_query(StateFilter(FSMFillForm.chose_note))
async def process_output_note(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        text=users_db[callback.from_user.id]['notes'][callback.data]['title_note'],
    )
    if users_db[callback.from_user.id]['notes'][callback.data]['text']:
        for text in users_db[callback.from_user.id]['notes'][callback.data]['text']:
            await callback.message.answer(text= text)

    if 'images' in users_db[callback.from_user.id]['notes'][callback.data]:
        images = users_db[callback.from_user.id]['notes'][callback.data]['images'] 
        media_ph = [InputMediaPhoto(media=file_id) for file_id in images]
        await callback.bot.send_media_group(
            chat_id=callback.message.chat.id,
            media=media_ph)

    if 'video' in users_db[callback.from_user.id]['notes'][callback.data]:
        video = users_db[callback.from_user.id]['notes'][callback.data]['video'] 
        media_vd = [InputMediaVideo(media=file_id) for file_id in video]
        await callback.bot.send_media_group(
            chat_id=callback.message.chat.id,
            media= media_vd)

    if 'doc' in users_db[callback.from_user.id]['notes'][callback.data]:
        doc = users_db[callback.from_user.id]['notes'][callback.data]['doc'] 
...
async def process_edit_note_sent(message: Message, state: FSMContext):
    ...
    note = await state.get_data()
    users_db[message.from_user.id]['notes'][note['title_note']] = note['text_note']
    print(users_db[message.from_user.id]['notes'])
    await state.clear()
...
async def del_note(callback: CallbackQuery, state: FSMContext):
    del users_db[callback.from_user.id]['notes'][callback.data[:-4]]
